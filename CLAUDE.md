...
縦画面・フローティング仮想スティック。画面のどこを触れてもその位置にスティックが出現。

| 操作 | アクション |
|---|---|
| 左右ドラッグ | 移動 |
| 上フリック | ジャンプ（再度フリックで2段ジャンプ） |
| 下フリック | 必殺技 |
| タップ（短押し） | 通常攻撃 |
| PCキー | A/D=移動, Z=ジャンプ, X=攻撃, C=必殺 |

実装は `game.js` の `setupTouchControls()`（`#game-ui` 全面で pointer イベントを受け、`inputSeq` を加算）。
...
| 1.1.0 | ソロモード・ランキング・勝利数記録を追加 |
| 1.2.0 | ぷにこん方式の片手操作・縦画面対応・ノックバック強化 |
